...
function cardClickHandle(target) {
    if (!target.classList.contains('btn-close'))
        return;
    if (!confirm('선택하신 카드를 삭제 하시겠습니까?'))
        return;
}
function cardDragStartHandle(evt) {
    resetOption();
    if (evt.target.dataset.type !== 'card')
        return;
...
function cardDrop(evt) {
    if (!option.dragTarget)
        return;
    let cardWrap = evt.target;
    option.currColumn = getParentEl(cardWrap, '.todo-columns');
    if (!cardWrap.classList.contains('card-wrap'))
        cardWrap = option.currColumn.querySelector('.card-wrap');
...
export { cardClickHandle, cardDragStartHandle, cardDragover, cardDrop, };
